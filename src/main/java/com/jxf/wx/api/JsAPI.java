...
import com.jxf.wx.api.response.GetSignatureResponse;
...
//    }
     ...
     * @return 签名以及相关参数
     */
    public GetSignatureResponse getSignature(String jsApiTicket,String url) {
        Assert.notNull(url,"URL不能为空");
        //当前时间的秒数
        long timestame = System.currentTimeMillis() / 1000;
        //使用UUID来当随机字符串
        String nonceStr = UUID.randomUUID().toString().replaceAll("-", "");
        return getSignature(jsApiTicket,nonceStr, timestame, url);
    }
     ...
     */
    // 
    public GetSignatureResponse getSignature(String jsApiTicket,String nonceStr, long timestame, String url) {
    	Assert.notNull(url,"URL不能为空");
