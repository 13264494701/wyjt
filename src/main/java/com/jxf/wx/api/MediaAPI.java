...
//    }
    ...
    public UploadMediaResponse uploadMedia(MediaType type, File file) {
    	...
    	response = JSON.parseObject(resultJson, UploadMediaResponse.class);
        return response;
    }
     ...
     * @return 响应对象
     */
    public UploadMediaResponse uploadNews(List<ArticleTemp> articles){
        UploadMediaResponse response;
     ...
     * 上传群发文章和永久图文消息中的图片
     */
    public UploadImageResponse uploadImage(File file){
    	UploadImageResponse response;
    	String url = BASE_API_URL + "cgi-bin/media/uploadimg?access_token=#";
    	BaseResponse r = executePost(url, null, file);
    	String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : JSON.toJSONString(r);
    	response = JSON.parseObject(resultJson, UploadImageResponse.class);   
    	return response;
    }
