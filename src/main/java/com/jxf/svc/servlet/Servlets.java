...
public class Servlets {
	...
	//不经过拦截的地址
	private final static String[] excludeUrls = StringUtils.split(Global.getConfig("web.exclude.regUrls"),",");
...
//		}
    ...
    public  static boolean isExcludeUrl(String url){
		if (excludeUrls == null){
			try {
				throw new Exception("检测到没有配置“web.exclude.regUrls”属性。配置示例：\n#静态文件后缀\n"
					+"web.exclude.regUrls=/base/*,/common/*");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		if(StringUtils.startsWithAny(url, excludeUrls)){
			return true;
		}
    	return false;
    }
