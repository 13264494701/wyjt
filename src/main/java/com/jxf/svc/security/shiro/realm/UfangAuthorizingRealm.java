 ...
 */
@Service
public class UfangAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (this.isValidateCodeLogin(token.getUsername(), false, false)){
			Session session = UfangUserUtils.getSession();
		...
		// 校验用户名密码
		UfangUser user = userService.getUserByUsername(token.getUsername());
		if (user != null) {
//			if (Global.NO.equals(user.getLoginFlag())){
//				throw new AuthenticationException("msg:该已帐号禁止登录.");
//			}
			if (user.getIsLocked()) {
				throw new AuthenticationException("msg:帐号已被锁定");
			}
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,16));
			return new SimpleAuthenticationInfo(new Principal(user.getId(), user.getUsername(),Principal.UserType.ufang), 
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (!Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))){
			Collection<Session> sessions = sessionDAO.getActiveSessions(true, principal, UfangUserUtils.getSession());
			if (sessions.size() > 0){
				// 如果是登录进来的，则踢出已在线用户
				if (UfangUserUtils.getSubject().isAuthenticated()){
				...
				// 记住我进来的，并且当前用户已登录，则退出当前用户提示信息。
					...
					UfangUserUtils.getSubject().logout();
					throw new AuthenticationException("msg:账号已在其它地方登录，请重新登录。");
				}
			}
		}
		UfangUser user = userService.getUserByUsername(principal.getLoginName());
		if (user != null) {
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			List<UfangMenu> list = UfangUserUtils.getMenuList();
			for (UfangMenu menu : list){
			...
			// 添加用户权限
			info.addStringPermission("ufang");
			// 更新登录IP和时间
			userService.updateUserLoginInfo(user);
			// 记录登录日志
			LogUtils.saveLog(Servlets.getRequest(), "优放用户登录");
			return info;
		} else {
			throw new UnknownAccountException();//没找到帐号  
		}
