package com.jxf.svc.persistence.interceptor;
...
//        final String countSql = "select count(*) from (" + sql + ") tmp_count";
//        final String countSql = "select count(*) " + removeSelect(removeOrders(sql));

    	int index = StringUtils.indexOfIgnoreCase(sql, "from");
    	String leftSql = sql.substring(0, index);
    	
    	final String countSql = StringUtils.contains(leftSql, "?")? "select count(*) from (" + sql + ") tmp_count" : "select count(*) " +sql.substring(index); 
