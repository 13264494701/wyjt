 ...
 * @功能说明:流水金额为0 问题   
 */
public class UpdateMemberTrx2017Utils {
	
	private static Logger log = LoggerFactory.getLogger(UpdateMemberTrx2017Utils.class);
	...
	public static int updateTrx(long startId, long endId)  {
		...
		String trxQuery = "select * from  MEM_MEMBER_ACT_TRX_2017  where org_id = 1 AND id >= ? AND id <=? ORDER BY id" ;
		
		String updateActSql = "UPDATE MEM_MEMBER_ACT_TRX_2017  SET org_id = ? where id = ?" ;
