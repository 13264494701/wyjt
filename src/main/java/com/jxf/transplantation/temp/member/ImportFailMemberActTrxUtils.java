 ...
 * @功能说明:会员的账单流水迁移
 */
public class ImportFailMemberActTrxUtils {
	
	private static Logger log = LoggerFactory.getLogger(ImportFailMemberActTrxUtils.class);
	
	public static void main(String[] args) {
		importTrx();
	}
    
	public static int importTrx() {
		...
		ResultSet rs = null;
		String countsql = " SELECT * from t_fund_detail a where a.userId in ('1235156','1251579')";
		...
		try {
			...
			String insertTrxSql = " INSERT INTO `MEM_MEMBER_ACT_TRX` ("
					+"`id`,"//1
					+"`trx_code`,"//2
					+"`member_id`,"//3
					+"`sub_no`,"//4
