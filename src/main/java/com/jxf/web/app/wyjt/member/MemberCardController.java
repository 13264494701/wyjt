...
import com.jxf.web.model.wyjt.app.card.BindBankCardRequestParam;
import com.jxf.web.model.wyjt.app.card.ChangeBankCardRequestParam;
 ...
 * @author gaobo
 */
@Controller("wyjtAppMemberCardController")
@RequestMapping(value="${wyjtApp}/card")
public class MemberCardController extends BaseController {
	...
	public @ResponseBody ResponseData bindBankCard(HttpServletRequest request){
		Member member = memberService.getCurrent();
		String param = request.getParameter("param");
		BindBankCardRequestParam reqData = JSONObject.parseObject(param, BindBankCardRequestParam.class);
	...
	public @ResponseBody ResponseData changeBankCard(HttpServletRequest request){
		Member member = memberService.getCurrent();
		String param = request.getParameter("param");
		ChangeBankCardRequestParam reqData = JSONObject.parseObject(param, ChangeBankCardRequestParam.class);
		MemberCard memberCard = memberCardService.getCardByMember(member);
		...
		if(StringUtils.equals(memberCard.getCardNo(), cardNo)) {
			logger.error("=====更换的银行卡与原有银行卡一致=====");
			return ResponseData.error("更换银行卡不能与原有银行卡一致");
		}
		...
		/**更换银行卡前 验证四要素*/
		ResponseData responseData = memberVerifiedService.verifyChangeCardInformation(cardNo, phoneNo, name, idNo, member);
		return responseData;
	}
