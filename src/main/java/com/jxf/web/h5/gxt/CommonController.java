 ...
 * @版本：V1.0
 */
@Controller("gxtH5CommonController")
@RequestMapping(value = "${gxtH5}/common")
public class CommonController {
    ...
    public @ResponseBody
    ResponseData sendSmsVerifyCode(String tmplCode, String phoneNo) {
        //生成短信验证码
        String smsCode = VerifyCodeUtils.genSmsValidCode();
        //将短信验证码存入缓存中
    ...
    public @ResponseBody
    ResponseData checkSmsCode(String smsCode, String phoneNo) {
	...
	public @ResponseBody
	ResponseData checkPhone(HttpServletRequest request, String phoneNo){
	...
	public String download(HttpServletRequest request) {
		...
		if (agent.contains("IPHONE")) {
			return "redirect:https://itunes.apple.com/cn/app/you-xin-bao-dian-zi-jie-tiao/id668353576?l=zh&ls=1&mt=8";
		}
		return "redirect:" + Global.getConfig("domain") + "/download/wyjt.apk";
	}
		 ...
		 String imageUploadPath = "";
		    ...
		    try {
		     for (Iterator<String>  it = multiRequest.getFileNames(); it.hasNext(); ) {
				...
				logger.error("视频上传异常：{}",Exceptions.getStackTraceAsString(e));
	...
	public ResponseData uploadImage(MultipartHttpServletRequest multiRequest,HttpServletResponse response) {
	    ...
	    try {
	     for (Iterator<String>  it = multiRequest.getFileNames(); it.hasNext(); ) {
			...
			multipartFile.transferTo(tempFile);	
			//Servlets.getRequest().getContextPath() 需要在网络请求的线程里面才能取到
            imageUploadPath = Global.getConfig("domain")+Servlets.getRequest().getContextPath()+FileUploadUtils.upload(tempFile, "arbitrationProof", "image", extension);
            pathList.add(imageUploadPath);
	     }
		} catch (IllegalStateException | IOException e) {
			logger.error("图片上传异常：{}",Exceptions.getStackTraceAsString(e));
			return ResponseData.error("图片上传错误");
		}
	    Map<String, Object> data = new HashMap<String,Object>();
	    data.put("imageUploadPath", pathList);
	    return ResponseData.success("上传成功", data);		
	}
