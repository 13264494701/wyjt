...
public class SendWithdrawTask implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		//处理待打款的订单
		sendPendingWithdrawOrder(teshuid,withdrawType);
		
		//处理连连重复订单 等连连订单处理完毕再取消任务
		confirmRepeatWithdrawOrder(teshuid);
		
	}
    
    private void sendPendingWithdrawOrder(String teshuid,String withdrawType) {
    	logger.info("提现-处理待打款订单");
		Calendar cal = Calendar.getInstance();
	 	cal.add(Calendar.DATE, -5);   
		NfsWdrlRecord nfsWdrlRecord = new NfsWdrlRecord();
		nfsWdrlRecord.setCreateTime(cal.getTime());
		nfsWdrlRecord.setStatus(NfsWdrlRecord.Status.pendingPay);
		List<NfsWdrlRecord> list = wdrlRecordService.findListByStatus(nfsWdrlRecord);
		logger.info("待打款提现订单共{}笔",list.size());
		for(NfsWdrlRecord wdrlRecord :list){
				try{
					if(teshuid.indexOf((","+wdrlRecord.getMember().getId()+","))!=-1){
						logger.error("特殊id提现失败-id: "+wdrlRecord.getMember().getId());
						continue;
					}
					if(wdrlRecord.getAmount().compareTo(StringUtils.toDecimal(wdrlRecord.getFee())) <= 0) {
						logger.error("提现金额小于手续费,提现失败-提现记录ID: " + wdrlRecord.getId());
						continue;
					}
					//发送连连请求
					if(StringUtils.equals(withdrawType, "fuiou")) {
						fuiouWithdrawService.sendFuiouPaymentRequestForAPP(wdrlRecord);
					}else if(StringUtils.equals(withdrawType, "lianlian")){
						lianlianPayService.sendLianLianPaymentRequestForAPP(wdrlRecord);
					}else {
						logger.error("没有配置提现方式！");
					}
				}catch (Exception e){
					logger.error("提现失败-提现记录ID: " + wdrlRecord.getId() + "异常："+ Exceptions.getStackTraceAsString(e));
				}
		}
    }
    private void confirmRepeatWithdrawOrder(String teshuid) {
		//处理重复订单 
		Calendar cal = Calendar.getInstance();
	 	cal.add(Calendar.DATE, -5);
	 	//查找疑似重复订单
	 	NfsWdrlRecord nfsWdrlRecord1 = new NfsWdrlRecord();
	 	nfsWdrlRecord1.setCreateTime(cal.getTime());
	 	nfsWdrlRecord1.setStatus(Status.mayRepeatOrder);
	 	nfsWdrlRecord1.setType(Type.lianlian);
	 	List<NfsWdrlRecord> wdrlRecords1 = wdrlRecordService.findListByStatus(nfsWdrlRecord1);
	 	for (NfsWdrlRecord nfsWdrlRecord : wdrlRecords1) {
			nfsWdrlRecord.setStatus(NfsWdrlRecord.Status.sendRepeatOrder);
			wdrlRecordService.save(nfsWdrlRecord);
		}
	 	//有可能人工会审核通过这些订单，需要重新查下待发送状态的，或者以后把疑似重复的审核功能屏蔽
	 	NfsWdrlRecord nfsWdrlRecord = new NfsWdrlRecord();
	 	nfsWdrlRecord.setCreateTime(cal.getTime());
	 	nfsWdrlRecord.setStatus(Status.sendRepeatOrder);
	 	nfsWdrlRecord.setType(Type.lianlian);
		List<NfsWdrlRecord> wdrlRecords = wdrlRecordService.findListByStatus(nfsWdrlRecord);
		logger.info("连连重复订单数: " + wdrlRecords.size());
		for (NfsWdrlRecord wdrlRecord : wdrlRecords) {
			if(teshuid.indexOf((","+wdrlRecord.getMember().getId()+",")) != -1){
				logger.error("连连提现-特殊id提现失败-id: "+ wdrlRecord.getMember().getId());
				continue;
			}
			if(wdrlRecord.getStatus().equals(Status.sendRepeatOrder)){
				try {
					lianlianPayService.confirmLianLianPayment(wdrlRecord.getId()+"",wdrlRecord.getRmk());
				} catch (Exception e) {
