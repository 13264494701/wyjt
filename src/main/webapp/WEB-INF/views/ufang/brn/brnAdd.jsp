...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ufang}/brn/add?id=${brn.id}&parent.id=${brn.parent.id}">机构添加</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="brn" action="${ufang}/brn/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<c:if test="${brn.grade != '1'}">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="area" name="area.id" value="${brn.area.id}" labelName="area.name" labelValue="${brn.area.name}"
					title="区域" baseUrl="${ufang}" url="/area/treeData" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="phoneNo" htmlEscape="false" style="width:250px;" maxlength="11"/>
			</div>
		</div>
