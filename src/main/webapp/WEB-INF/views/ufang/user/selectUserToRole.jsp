...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			brnTree = $.fn.zTree.init($("#brnTree"), setting, brnNodes);
		...
		var brnNodes=[
	            <c:forEach items="${brnList}" var="brn">
	            {id:"${brn.id}",
	             pId:"${not empty brn.parent?brn.parent.id:0}", 
	             name:"${brn.name}"},
	            </c:forEach>];
		...
		function treeOnClick(event, treeId, treeNode, clickFlag){
			...
			if("brnTree"==treeId){
				$.get("${admin}/sys/role/users?brnId=" + treeNode.id, function(userNodes){
...
<!-- 		<div class="span4" style="border-right: 1px solid #A8A8A8;">
			<p>所在部门：</p>
			<div id="brnTree" class="ztree"></div>
		</div> 
-->
		<div class="span3">
			<p>待选人员：</p>
