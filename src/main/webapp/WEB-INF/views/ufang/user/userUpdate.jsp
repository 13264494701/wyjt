...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ufang}/user/update?id=${user.id}">员工修改</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="user" action="${ufang}/user/update" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="brn" name="brn.id" value="${user.brn.id}" labelName="brn.brnName" labelValue="${user.brn.brnName}"
					title="部门" baseUrl="${ufang}" url="/brn/treeData?type=2" cssClass="required"  disabled="disabled" notAllowSelectParent="true"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:checkboxes disabled="${user.type=='predefine'}" path="roleIdList" items="${allRoles}" itemLabel="roleName" itemValue="id" htmlEscape="false" class="required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
