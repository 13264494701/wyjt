...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/brn/nfsBrnAct/">机构账户列表</a></li>
		<li class="active"><a href="${ctx}/brn/nfsBrnAct/query?id=${nfsBrnAct.id}">机构账户查看</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="nfsBrnAct"  method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<input name="createTime" type="text" style="width:230px"  disabled="disabled" maxlength="20" class="input-medium Wdate "
				value="<fmt:formatDate value="${nfsBrnAct.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			...
			<input name="updateTime" type="text" style="width:230px"  disabled="disabled" maxlength="20" class="input-medium Wdate "
				value="<fmt:formatDate value="${nfsBrnAct.updateTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:true});"/>
			<span class="help-inline"  style="width:150px"></span>	
		</div>
		<div class="form-actions">
		    <a href="${ctx}/nfsBrnAct/delete?id=${nfsBrnAct.id}" class="btn btn-primary " style="width:60px;" type="button" onclick="return confirmx('确认要删除该机构账户吗？', this.href)">删除</a>
