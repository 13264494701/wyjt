...
<h1>
	<c:choose> 
	  <c:when test="${authItem=='sesame_multiple'}">   
	    	芝麻分查询
	  </c:when> 
	  <c:when test="${subItem=='jdb'}">
	    	借贷宝用户分析
	  </c:when> 
	  <c:when test="${subItem=='hhd'}">
	    	米房用户分析
	  </c:when> 
	  <c:when test="${subItem=='jjd'}">
	    	今借到用户分析
	  </c:when> 
	</c:choose> 
</h1>
<form action="${ufang}/rcGxb/getToken" method="get" style="margin-top: 30px" onsubmit="return check()">
    <input type="hidden" value="${authItem}" name="authItem">
    <input type="hidden" value="${subItem}" name="subItem">
    <div>
